...
class excelCheck
{
  ...
  {
    //echo ' </br> A classe "', __CLASS__, '" foi destruída.<br />';
  }
  ...
  public function verificarConteudo(){
    ...
    if($ext == 'xls'){
      ...
      //echo '<pre>'; print_r($data->sheetNames());echo '</pre>'; 
      $cells = $data->rows(1); 
      foreach($cells as $col => $cell){
        if($col == 0){
          $fields = $cell;                                
          ...
          $row = array();
          foreach($fields as $findex => $fval){
                                  //echo $fval . "=> " . ($cell[$findex]?$cell[$findex]:"NULL") . " <> ";
            $row[$fval] = (isset($cell[$findex])?$cell[$findex]:NULL);
          }
                                //print_r($row);
          $rs[] = $row;
      ...
      //echo "<pre>";  print_r($this->fields_obri);   echo "<pre>";
